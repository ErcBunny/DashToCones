...
class BaseController:
	...
	self.sonar_pub_cloud = rospy.Publisher("/sonar_cloudpoint", PointCloud2)
    ...
    def poll(self):
		...
		pcloud = PointCloud2()
		    ...
		    if sonar1_range.range>=0.5 or sonar1_range.range == 0.0:
                    ...
                    rospy.logerr("ping_o exception count: " + str(self.bad_encoder_count))
                    return
		    ...
		    if sonar2_range.range == 0.0:  #sonar error or not exist flag
  		    ...
  		    if sonar2_range.range>=0.5 or sonar2_range.range == 0.0:
	            ...
	            if sonar3_range.range>=0.5 or sonar3_range.range == 0.0:
                    ...
                    self.bad_encoder_count += 1
                    rospy.logerr("ping_p exception count: " + str(self.bad_encoder_count))
                    return
		    ...
		    if sonar4_range.range>=0.5 or sonar4_range.range == 0.0:
                    ...
                    self.bad_encoder_count += 1
                    rospy.logerr("ping_q exception count: " + str(self.bad_encoder_count))
                    return
    ...
    def cmdVelCallback(self, req):
		...
		temp = self.sonar_r0/(self.safe_range_1*5.0)
		if(x<-0.1):
			x= temp*x;
			if(x>-0.05):
				x=-0.05	
		...
		temp = self.sonar_r1/(self.safe_range_1*5.0)
		if(th < -0.05):
			th = temp*th
	    if(self.sonar_r2 >self.safe_range_0 and self.sonar_r2 <= self.safe_range_1 and x>0):
		temp = self.sonar_r2/(self.safe_range_1*5.0)
	    	if(x>0.1):
			x= temp*x;
			if(x<0.05):
				x=0.05
	    if(self.sonar_r3 >self.safe_range_0 and self.sonar_r3 <= self.safe_range_1 and th >0):
		temp = self.sonar_r3/(self.safe_range_1*5.0)
		if(th>0.05):
			th = temp*th;

	    if(self.sonar_r4 >self.safe_range_0 and self.sonar_r4 <= self.safe_range_1 and x <0):
		temp = self.sonar_r4/(self.safe_range_1*5.0)
		if(x<-0.1):
			x= temp*x;
			if(x>-0.05):
				x=-0.05	
	    ...
	    if((self.sonar_r3<=self.safe_range_0 and self.sonar_r3>=2) and (x>0)):
            ...
            if((self.sonar_r3<=self.safe_range_0 and self.sonar_r3>=2) and (th>0)):
                x=-0.1
                th=-0.2
            if((self.sonar_r4<=self.safe_range_0 and self.sonar_r4>=2) and (x<0)):		
                x= 0.0
...
#            x = 0
#            if self.sonar_r2>=0.1:
#                th = 0.5
#
#            elif self.sonar_r3>=0.1:
#                th = -0.5
#            elif self.sonar_r4>=0.2:
#                x = -0.1
#                th = 0
	    ...
	    if (th>-0.2 and th<0.0) and (x>-0.05 and x<0.05):
            ...
            #print "33333gaibianhou w= "+str(th)+"v= "+str(x)
